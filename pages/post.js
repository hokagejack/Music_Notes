    ...
    <div className="my-20 p-12 shadow-lg rounded-lg max-w-med mx-auto">
      <form onSubmit={submitPost}>
        ...
        <div className="py-2">
          ...
          <p className={`text-green-500 font-medium text-sm ${post.description.length > 300 ? `text-red-600` : ""}`}>{post.description.length}/300</p>
        </div>
        <button 
        type="submit"
        className="v-full bg-green-600 hover:bg-green-500 active:bg-green-200 text-white font-medium my-2 p-2 rounded-lg text-sm">Submit</button>
      </form>
    </div>
  );
}